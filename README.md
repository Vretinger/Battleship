...
[Press here to play the live version of the game!](https://ci-battleship-8283296ac1b0.herokuapp.com/)

![Battleship Game](Images/BattleShipShowcaseImage.png)
...
### Placing Ships:
- The player places ships on their board by entering coordinates and directions or selecting random placement.
- The computer places ships randomly.

### Gameplay:
- Players take turns attacking by guessing the opponent's ship locations.
- Enter coordinates (e.g., H5) to make an attack.
- The board will update to show hits (X) and misses (O).
- If a player hits the enemy ships, they will continue attacking until they miss.
...
- **Player and Computer Boards:** Separate boards for the player and the computer.
  ...
  ![Game Boards](Images/GameBoards.png)

- **Ship Placement:** Place ships manually or randomly.
  
  ![Ship Placement Options](Images/PlacementOption.png)
...
- **Different Difficulty Levels:** Players can choose between three different difficulties.
  - **Easy:** In easy mode, the computer guesses randomly, even after a hit.
  - **Hard:** In hard mode, the computer guesses random locations around the hit location until the ship is sunk.
  - **Expert:** In expert mode, the computer determines the direction of the ship after the second hit and attacks along that line until the ship is sunk.
  
  ![Difficulty Levels](Images/DifficultyLevels.png)

- **Clear Terminal Screen:** The terminal screen is cleared to enhance the gameplay experience.

- **Accept User Input:**
  
  ![User Input](Images/EnterInput.png)
...
- **Validate the Input:**
  ...
  ![Validate Input](Images/ValidateInput.png)
...
- **npc_ship_board and player_ship_board:** Board objects tracking the ships' IDs on the NPC’s and player’s boards to determine when a ship is sunk.
...
| Feature                | Expected Result                                           | Steps Taken                                    | Actual Result |
|------------------------|-----------------------------------------------------------|------------------------------------------------|---------------|
| **Game Flow**              | Smooth transition from the main menu to gameplay and end-game scenarios. | Navigated through the game from start to end. | As Expected    |
...
To deploy the Battleship game to Heroku, follow these steps:
...
To fork the repository and create a personal copy:
